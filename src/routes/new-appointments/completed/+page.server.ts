...
export const actions = {
    default: async ({ request }) => {
        ...
        const new_appointment_full_date = (processedData.new_appointment_date && processedData.new_appointment_time) ? 
            ...
            null
        if(!processedData.patient_id || !processedData.form_id || !processedData.appointment_id) 
        ...
        if(isNewPayment && isNewAppointment) {
            ...
            const { error: dbErr } = await supabase
                ...
                .insert([{
                    ...
                    amount: processedData.payment_amount,
                    is_amount_positive: processedData.is_amount_positive,
                    appointment_id: processedData.appointment_id
                }])
            if(dbErr) {
                console.log(dbErr);
                redirect(303, '/dbError');
